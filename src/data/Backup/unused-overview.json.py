...
for db in databases:
    ...
    query="""
    SELECT raidId, spec, dps, boss, difficulty, gearScore, arkPassiveActive FROM players
...
# Clean up
main_df["arkPassiveActive"]=main_df["arkPassiveActive"].fillna(-1).astype(int).map({-1:"All",0:"Off",1:"On"})
